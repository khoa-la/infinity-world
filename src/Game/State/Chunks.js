...
export default class Chunks {
  ...
  check() {
    ...
    for (const [key, chunk] of this.mainChunks) {
      if (
        !mainChunksCoordinates.find((coordinates) => coordinates.key === key)
      ) {
        chunk.destroy();
  ...
  update() {
    ...
    const playerChunkKey = `${Math.round(
      (player.position.current[0] / this.minSize) * 2 + 0.5
    )}${Math.round((player.position.current[2] / this.minSize) * 2 + 0.5)}`;
  ...
  updateAllNeighbours() {
    ...
    for (const chunk of chunks) {
       ...
       */
      // From quad
      if (chunk.quadPosition === "sw") nChunk = chunk.parent.children.get("nw");
      // From quad
       ...
       */
      // From quad
      if (chunk.quadPosition === "nw") eChunk = chunk.parent.children.get("ne");
      // From quad
       ...
       */
      // From quad
      if (chunk.quadPosition === "nw") sChunk = chunk.parent.children.get("sw");
      // From quad
       ...
       */
      // From quad
      if (chunk.quadPosition === "ne") wChunk = chunk.parent.children.get("nw");
      // From quad
      else if (chunk.quadPosition === "se")
