...
export default class Noises {
  ...
  setHelper() {
    ...
    const meshA = new THREE.Mesh(this.helper.geometry, this.helper.material);
    meshA.position.y = 5 + 1;
    meshA.position.x = -1;
    meshA.scale.set(2, 2, 2);

    const meshB = new THREE.Mesh(this.helper.geometry, this.helper.material);
    meshB.position.y = 5 + 1;
    meshB.position.x = 1;
    meshB.scale.set(2, 2, 2);

    const meshC = new THREE.Mesh(this.helper.geometry, this.helper.material);
    meshC.position.y = 5 - 1;
    meshC.position.x = -1;
    meshC.scale.set(2, 2, 2);

    const meshD = new THREE.Mesh(this.helper.geometry, this.helper.material);
